...
1. Для выполнения работы достаточно реализовать только поддержку заголовков 8-битных BMP (цветных и в оттенках серого), поставляемых в составе задания (дополнительные проверочные изображения, на которых будет проверятся программа, созданы в тех же приложениях, что и изображения из состава задания и должны открываться также, как и изображения в составе задания);
2. BMP поддерживает алгоритмы сжатия - их реализовывать не нужно.
3. Некоторые форматы изображения поддерживают различные схемы ориентации изображения в файле. Например, формат TIF поддерживает хранение изображений в повёрнутом на 90, 180 или 270 градусов виде, а также в горизонтальном или вертикальном зеркальном отражении. РАСТР BMP-ИЗОБРАЖЕНИЯ ХРАНИТСЯ, ПО УМОЛЧАНИЮ, ПЕРЕВЁРНУТЫМ – первая строка в матрице растра является последней (нижней) строкой в изображении, а последняя строка в матрице растра является первой (верхней) строкой в изображении.
4. Строки изображения BMP выровнены по 4 байта - если количество байт, занимаемых всеми пикселями строки, не кратно 4, в конце строки добавляется необходимое число "мусорных" байтов.
5. При выполнении работы для улучшения понимания внутреннего устройства файлов могут помочь hex-редакторы, позволяющие открывать и просматривать любые файлы побайтово. 
	Распространёнными hex-редакторами в Windows являются:
	а) HxD;
	б) Notepad++ с установленным плагином HEX-editor (Плагины (Plugins) -> Управление плагинами -> ...).
